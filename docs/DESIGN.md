...
The first J4 shift is a supervised qualification experience. Temporary supervisor callouts and highlights focus the player on the Contact Dossier, relevant dossier fields, Regulations reference, allegation controls, ruling console, Ops Log, Hold Tomography, and the post-scan MARK then DETAIN flow as those concepts become relevant. These prompts should reduce first-play overwhelm while preserving normal interaction and avoiding hidden-truth verdicts. Development and repeat-play sessions can bypass these prompts with `?skip-tutorial=true`.
